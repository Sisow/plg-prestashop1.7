...
class SisowPaymentModuleFrontController extends ModuleFrontController
{
    ...
    private function checkCoc(&$coc, $company) {
        if (!empty($coc) && is_numeric($coc)) {
            if (empty($company)) {
                $this->errors[] = 'Bedrijfsnaam is verplicht bij een zakelijke betaling';
            ...
            if (empty($coc) || !is_numeric($coc)) {
                $this->errors[] = 'KVK nummer is verplicht bij een zakelijke betaling';
    ...
    public function postProcess()
    {
        ...
        $shippingCostExTax = $this->context->cart->getOrderTotal(false, Cart::ONLY_SHIPPING);
        /*
        ...
        */
		...
		if($shippingCost > 0 && $shippingCostExTax > 0){
		...
		else if($payment == 'afterpay')
		{
            ...
            $this->checkCoc($coc,$billingAddress->company);
